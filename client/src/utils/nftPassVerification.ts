...
/**
 * Check if a wallet owns a SolStudio Platform Pass NFT
 */
export async function checkNFTPass(
  ...
  try {
    ...
    // Check each NFT to see if it's a SolStudio Platform Pass
    for (const nftAccount of nftAccounts) {
      ...
      try {
        ...
        if (metadata) {
          // Parse metadata to check if it's a SolStudio Platform Pass
          const metadataDecoded = parseMetadata(metadata.data);
          
          if (isSolStudioPlatformPass(metadataDecoded)) {
            console.log('Found SolStudio Platform Pass!');
            return {
              hasPass: true,
              nftMint: mintAddress,
    ...
    console.log('No SolStudio Platform Pass found');
...
/**
 * Check if the metadata indicates this is a SolStudio Platform Pass
 */
function isSolStudioPlatformPass(metadata: any): boolean {
  ...
  // Check for SolStudio Platform Pass indicators
  return name.includes('sol studio platform pass') || 
         name.includes('solstudio platform pass') ||
         name.includes('solstudio pass') ||
         symbol.includes('solxpass') ||
         symbol.includes('solstudio') ||
